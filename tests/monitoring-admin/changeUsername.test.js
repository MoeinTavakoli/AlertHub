...
const body = {
  newUsername: 'new.user.monitor'
};



const username = 'user.monitor';
...
test('change username without any argument (error schema validator)', async () => {
  const response = await request(app).put(`/monitoring-admin/username/${username}`)
...
test('change username after change prev username ', async () => {
  const response = await request(app).put(`/monitoring-admin/username/${username}`)
    ...
    .set('Authorization', monitoringAdminToken);
  expect(response.status).toBe(400);
  expect(response.text).toBe('didnt change username !!!');
});
