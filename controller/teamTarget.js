...
async function createTeamTarget(req, res) {
  try {
    const { targetAddress, teamName } = req.body;
    const result = await db.createTeamTarget(targetAddress, teamName);
...
async function removeTeamTarget(req, res) {
  try {
    const { targetAddress, teamName } = req.body;
    const result = await db.removeTeamFromTarget(targetAddress, teamName);
